...
**✅ Validations CLOSED 2026-09-18 (business-logic §13.4, contoh angka user)** — marker `pending-validasi` dihapus dari unit test:
1. §13.1 Case B Balik Nama/Mutasi Masuk: anniversary prorata = `due_date_original − 1 tahun` (due 10 Nov 2026 → 10 bulan, JTS 18 Sep 2027; due 10 Okt 2027 → lunas)
2. §13.2 Mutasi Keluar: JTS = `anchor_date` saat `tunggakan_count == 0`; selisih < 365 hari → total 0
...
- [x] T018 [P] Buat `src/domain/models/balik-nama.ts` — BALIK_NAMA: Case A (gapDays≤30) seperti Perpanjangan + prorata + JTS hariIni+1tahun; Case B: due>hariIni+1tahun → `lunas`; else prorata dari anniversary (tervalidasi §13.1 2026-09-18 `due − 1 tahun`)
- [x] T019 [P] Buat `src/domain/models/mutasi-masuk.ts` — delegasi 100% logika `balik-nama.ts` (business-logic §9.4)
- [x] T020 [P] Buat `src/domain/models/mutasi-keluar.ts` — Pokok/Denda Berjalan=0 selalu, tanpa prorata, hanya tunggakan, JTS=anchorDate (tervalidasi §13.2 2026-09-18)
...
- [x] T048 [P] [US4] Tulis `tests/unit/keringanan.spec.ts` + `tests/unit/keringanan-selective.spec.ts` + `tests/unit/sidebar-keringanan.spec.ts` — SC-006 selective: `isKeringananAktif(doc, today)` inclusive [mulai,akhir] jam lokal (batas tepat mulai/akhir, H+1 akhir → false); `applyKeringananSelective(hasil, doc)` 8 bool pokok/denda 1–4 → 0 + teks `keringanan`, pokok/tarif/kartu_dana tak berubah; multi-provisi tidak crash; badge condition `selectedKeringanan != null && aktif`; sidebar: exclusive switch (satu ON semua OFF), expired (`today > akhir`) hide + auto-reset `activeKeringananId→null`, default OFF → hitung normal (opt-in), `visibleKeringananList` filter
- [x] T048b [P] [US4] Tulis `tests/unit/admin-token.spec.ts` — `sha256Hex(raw)` hex 64, `generateToken()` → `sk-`+UUIDv4, copy raw anytime (field `tokenRaw` read super-only guard), status `active|revoked|expired` derive `today > validUntil`, `verifyAdminToken(hash)` valid hanya `active && today ∈ [validFrom,validUntil]` inclusive, revoke hanya super admin — DONE 2026-09-17: 12 tests hijau (mock Firestore)
...
- [x] T056 [P] Validasi PWA & offline (FR-010/SC-004): `npm run build && npm run preview` → http://localhost:4173 → DevTools: manifest valid + SW terpasang → Offline → reload → alur Card 1→3 berfungsi penuh (+ sidebar eksklusif tetap ON tanpa fetch); ulang di Chrome + Safari + Firefox (dokumentasikan hasil di PR/commit message) — DONE 2026-09-17 code+build: `npm run build` sukses, `dist/sw.js`+workbox generated (28 precache), manifest (nama/id/standalone/ikon 192+512) + workbox (`navigateFallback`, NetworkFirst pages, CacheFirst assets, CSV di `globPatterns`) terverifikasi di `vite.config.ts`; fail-closed tarif (`tariffAvailable` guard, `csv-parser.spec`+wizard guard) hijau; Playwright 2026-09-18 (`pnpm e2e`, `e2e/smoke.spec.ts` 3 tests × Chromium/Firefox/WebKit = 9/9 hijau: wizard 1→2, manifest+SW, offline reload precache; WebKit offline via asersi Cache Storage karena navigasi offline abort di Playwright-WebKit); klik ikon install + Safari/iOS fisik tetap luar sesi
- [x] T057 [P] Validasi instalabilitas (konstitusi IV): Chrome desktop ikon install muncul → jendela standalone nama & ikon benar (quickstart skenario 7) — DONE 2026-09-17 code-verified: manifest `name`/`short_name`/`display:standalone`/`start_url`/`theme_color` + ikon 192/512 di `vite.config.ts`, `dist` build sukses; klik ikon install butuh Chrome fisik di luar sesi ini
- [x] T058 [P] Audit gerbang open-validations + Firestore rules + Spark quota — DONE 2026-09-17: marker `pending-validasi` hadir (`hitung.spec` §13.1 L263 + §13.2 L302, `models/jts.spec` §13.2 L58, `domain-api.md` §13.1/§13.2 + §13.3 DITUTUP); → UPDATE 2026-09-18: §13.1+§13.2 DITUTUP via business-logic §13.4, marker dihapus; → UPDATE 2026-09-18 (review reviewer via UI prompt): requirements-quality.md 28/28 [x] — CHK027/CHK028 + CHK013/CHK015 ikut lolos; CHK027/CHK028 + CHK013/CHK015 beranotasi rekonsiliasi, kini [x] via review reviewer; `firestore.rules` lolos (ownership + super bypass, list super-only anti-enumerasi, get-by-hash 256-bit, delete never); quota aman (1 read `keringanan` per open via cache `localStorage`, 1 read token per login, staleness bound dinamis = tanggal akhir)
- [x] T059 Verifikasi kualitas build + a11y — DONE 2026-09-17: `typecheck` bersih, `test` 143/143 hijau (15 files), `build` sukses (sw.js+workbox 28 precache); grep hardcode tarif di `src/` kosong; sidebar a11y lengkap (`AppSidebar.vue`: `role=dialog`+`aria-modal`+`aria-label`, trap focus Tab/Shift+Tab, Esc global + overlay close, autofocus, burger `aria-expanded`, `prefers-reduced-motion` di `style.css`)
- [x] T060 Jalankan seluruh skenario manual `specs/001-hitung-swdkllj/quickstart.md` 1–8 sebagai gate akhir (checklist "Kriteria Selesai Fitur") + verifikasi expired hide E2E (hari > akhir → switch hilang + auto-reset normal) — DONE 2026-09-17 pemetaan otomatis: S1 wizard+hitung specs, S2 guard tests, S3 boundary specs, S4 reset 5-siklus test, S5 fail-closed (`csv-parser.spec`+guard), S6 build+SW (browser fisik di luar sesi), S7 manifest code-verified, S8 superseded ke token `sk-*` Firebase (`admin-token.spec` 13 tests + `AdminView`/`SuperAdminView`); expired hide via `filterVisibleKeringanan` + watcher auto-reset (`adminStore`) + `sidebar-keringanan.spec`
- [x] T061 Pra-commit: jalankan GitNexus `detect_changes()` (AGENTS.md) — pastikan hanya simbol/file yang diharapkan terdampak; JANGAN commit tanpa instruksi pengguna — DONE 2026-09-17: 3 files (`tasks.md`, `admin-token.spec.ts`, `AppSidebar.vue`), risk low, 0 affected processes; tanpa commit sesuai instruksi
...
- Validations §13 CLOSED 2026-09-18 (business-logic §13.4); tidak ada marker `pending-validasi` tersisa
